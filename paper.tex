...
\begin{enumerate}[leftmargin=*,itemsep=2pt]
  ...
  \item \textbf{H3 --- multi-source coverage.} \textbf{MET (5 of 5
        ...
        N-terminal domain (PDB 1YER apo + 1YET heterogen-stripped
        for "holo-shape"; $\sim$207~CAs each). 3 $\times$ 300~ns
        MD per system + the same 4 generative ensembles (AlphaFlow,
        Boltz-2, BioEmu, MarS-FM) at 200 samples each. The result
        is the \emph{opposite} of Notch1: Hsp90 MD is a broad
        equilibrium explorer covering 3 of 4 VAMPnet states ($76\,\%$
        of MD frames), while AlphaFlow, Boltz-2, and BioEmu all
        \emph{collapse} to a single ``consensus'' state (RMSD
        $1.3$--$3$~\AA{} vs the 1YER apo crystal); MarS-FM uniquely
        also reaches one MD-extended state. A 6-source joint
        VAMPnet on apo + holo MD + 4 generative further shows that
        apo MD and holo MD distribute into different states
        ($88\,\%$ vs $100\,\%$ of frames respectively), with
        essentially zero overlap, and the 4 generative samplers
        \emph{all} cluster in the apo-MD basin. The v0.6 finding of

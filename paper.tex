...
\affil[1]{MedARC}
\date{\textit{v0.3 implementation report --- generated \today}}
...
\vspace{1em}
\noindent\textbf{Status (v0.3):} bundle shipped, three GitHub releases
tagged (\texttt{v0.1}, \texttt{v0.1.1}, \texttt{v0.2}); chignolin
Tier-1 validation passes; four-protein ATLAS robustness sweep passes;
LLM-agent adaptive sampling demonstration passes; Notch1 NRR apo +
corrected-chain holo MD complete under two restraint protocols on
Modal A100-80GB; the H2 directional prediction is met and replicates
across both protocols ($+3.7$ pp under v0.2 no-restraint, $+3.8$ pp
under v0.3 COM-distance restraint), but the pre-registered magnitudes
are blocked by the 100 ns sampling horizon (the COM restraint
eliminates the NRR-dissociation artifact without moving the
populations, isolating the cause). Recovering the magnitudes requires
generative ensembles (v0.4, MarS-FM is the natural target;
Sec.~\ref{sec:roadmap}).

...
\affil[1]{MedARC}
\date{\textit{v0.4 implementation report --- generated \today}}
...
\vspace{1em}
\noindent\textbf{Status (v0.4):} bundle shipped, four GitHub releases
...
v0.4 adds MarS-FM and BioEmu as additional ensemble sources, with
**H3 multi-source coverage MET**: state 2 of the joint VAMPnet ($9\,\%$
of the joint ensemble) is reached only by generative samples
($96.4\,\%$ from BioEmu, $3\,\%$ from MarS-FM, zero from MD) ---
direct evidence that generative ensembles surface conformations
classical 100\,ns MD does not reach. Bootstrap CIs on H2 indicate
the $+3.8$ pp directional Delta is not statistically significant at
$n = 3$ replicas (95\,\% CIs overlap). AlphaFlow and Boltz-2
integration deferred to v0.5 (upstream-dep-stack incompatibilities;
neither model is on the H3-critical path).
...
\subsection{Pre-registered hypotheses --- status as of v0.4}
...
\begin{enumerate}[leftmargin=*,itemsep=2pt]
  ...
  \item \textbf{H2 --- population shift.} \textbf{Partially met
        ...
        candidate).
  \item \textbf{H3 --- multi-source coverage.} \textbf{MET (3 of 3
        sources).} The v0.4 joint VAMPnet was fit on a 1869-frame
        ensemble combining three sources of Notch1 NEC (174 CAs):
        classical MD (1500 frames, v0.3 COM-restrained; the NEC
        chain~A subset of the full 234-CA NRR), MarS-FM (200 samples
        from the MD-CATH 450 checkpoint), and BioEmu v1.1 (169 samples
        from \texttt{microsoft/bioemu}). Per-state source breakdown:
        state 0 ($56\,\%$ pop) and state 3 ($22\,\%$) are MD-only;
        state 1 ($13\,\%$) is reached by all three sources
        (MarS-FM-dominated, $97\,\%$); state 2 ($9\,\%$) is
        \emph{generative-only} ($96.4\,\%$ from BioEmu, $3\,\%$ from
        MarS-FM, zero from MD). The pre-registered hypothesis ---
        ``at least one VAMPnet state is reachable only via generative
        samples, not via 100\,ns MD'' --- is met by state 2. Per-source
        inference cost on Modal: $\sim$\$0.10 per 200 MarS-FM samples
        (H100, $\sim$3 min) and $\sim$\$0.50 per 200 BioEmu samples
        (A100-80GB, $\sim$25 min). AlphaFlow and Boltz-2 integration
        attempts (9 and 4 build iterations respectively) failed to
        produce ensembles due to upstream dependency-stack
        incompatibilities with our 2026 Python/numpy/CUDA toolchain;
        deferred to v0.5 and not on the H3-critical path since H3 is
        already met.
\end{enumerate}
